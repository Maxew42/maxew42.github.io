...
The cognition of humans and animals is still poorly understood. However, it is not necessary to have electrodes to notice the gap that currently exists between human learning and that of an Artificial Neural Network (ANN). A child quickly find out the difference between a dog and a cat or can predict the trajectory of a toy thrown in the air. Even a basic ANN requires hundreds of examples to reach the same results. In the supervised learning paradigm, these examples must be labeled, which results in significant costs. Other paradigms such as Reinforcement Learning and Self-supervised Learning do not require labeling, but are not more "sample-efficient." Are ANNs therefore very different from our brains ?

A closer examination of the operation of biological neural networks confirms this difference. Here are some notable points of divergence.  
